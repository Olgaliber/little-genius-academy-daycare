...
switch( $post_format ) {
	...
	case 'gallery':
		...
		$images = explode( ",", get_post_meta($post->ID, 'gallery-images', true) );
		...
		if ( !empty($images) ) {
			...
			?><div class="post-thumb"><div id="banner-<?php echo $banner_rand; ?>" class="carousel slide">
				...
				<div class="carousel-inner"><?php
					...
					foreach( $images as $image ) {
						?><div class="item<?php if( !$slide ) { ?> active<?php } ?>"><?php echo wp_get_attachment_image($image, $thumb_size); ?></div><?php
						$slide++;
					}
